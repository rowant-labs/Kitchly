...
export const kitchenPlugin: Plugin = {
  ...
  init: async (_config: Record<string, string>, runtime) => {
    const apiKey = runtime.getSetting('INSTACART_API_KEY');
    if (!apiKey) {
      console.warn(
...
// Re-export service and types for downstream consumers
export { InstacartService } from './services/instacartService.js';
